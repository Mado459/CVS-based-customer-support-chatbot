...
# 4. Function to display chat messages
def display_chat():
...
# 5. Function to handle user input and generate assistant response
def handle_input():
...
# 6. Main execution
display_chat()  # Display existing chat messages
handle_input()  # Handle user input and generate response
...
# Note: This does not include indexed information (more on that below)
...
def generate_response(prompt, history, model_name, temperature):
    ...
    # After getting the chatbot's full response, we package it in a specific format and present it as the final result of this function.
